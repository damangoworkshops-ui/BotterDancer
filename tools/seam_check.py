...
def check_seam(grays, hists, seam, win=10):
    ...
    motion_ratio = float(diffs[i] / base) if base > 1e-6 else float("nan")
    ...
    return motion_ratio, color_ratio, {"diff_seam": float(diffs[i]), "diff_base": base,
...
def main():
    ...
    ap.add_argument("--max-motion-ratio", type=float, default=2.5,
                    help="Naht-Differenz / lokale Baseline (1.0 = unauffaellig)")
    ...
    for s in seams:
        if not (1 <= s < len(grays)):
            ...
            continue
        mr, cr, det = check_seam(grays, hists, s)
        flag = ""
        if mr > args.max_motion_ratio:
            flag += "  <-- BEWEGUNGSSPRUNG"
        if cr > args.max_color_ratio:
            flag += "  <-- FARB-/IDENTITAETSSPRUNG"
        if flag:
            bad.append(s)
        print(f"  Naht @{s:4d}: Bewegung {mr:.2f}x Baseline, Farbe {cr:.2f}x Baseline{flag}")
